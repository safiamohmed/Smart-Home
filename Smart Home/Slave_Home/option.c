...
// Declare ac_status as extern to use it across files
extern u8 ac_status;
u8 PIR_Check(void)
{
    u8 pir_status;
    DIO_u8GetPinValue(DIO_PORTB,DIO_PIN1,&pir_status);
    return pir_status; // Returns 1 if motion detected, 0 otherwise
}
...
void AC(u8 Copy_u8ROOM, u8 Copy_u8Status)
{
    if (Copy_u8ROOM == '1')
    {
        if (Copy_u8Status == '1')
        {
            ...
            ac_status = 1; // Update AC status to ON
        }
        else if (Copy_u8Status == '2')
        {
            DIO_u8SetPinDir(DIO_PORTC, DIO_PIN2, DIO_PIN_OUT);
            ...
            ac_status = 0; // Update AC status to OFF
        }
    }
    else if (Copy_u8ROOM == '2')
    {
